...
**Stats vs. analytics vs. exam (deliberately split pages).**
...
- `admin/exam.php` — *exam-mode usage* (FR-EXM-8): adoption, the start→finish funnel (finished/stopped/abandoned, rebuilt from `log` 36/37/38 via `additional_value = attempt_id` — stopped/abandoned attempts leave no `exam_attempts` row), grade distribution + pass rate + avg time, per-lecture difficulty (`exam_attempt_questions.max_lecture`), and retake/improvement (first-vs-latest grade). Read-only, portable SQL, no schema change; the funnel counts come from `log` while grades/per-lecture come from `exam_attempts`/`exam_attempt_questions`.
