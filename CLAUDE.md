...
- **Answer logging is two writes**: `recordAnswer()` updates `point_log`/`user_q`/`questions` and leveling but does **not** write the `log` audit row — the caller does (`writeLog(1|2, $qid)` in the practice `Q` handler and in `handleExamAnswer()`). A new answer path must mirror both, or the `log`-based analytics undercount. Post-answer badge checks are shared via `runAnswerBadgeChecks()`.
- **Exam mode** (`exam_functions.php`): exam answers route through `recordAnswer()` on purpose (they count as practice). The 20-min timer is enforced **lazily** (webhook has no clock); a dangling `in_progress` attempt is dropped on the next `startExam()`. "Stop" (`cancelExam()`) logs `ExamStopped` then **deletes** the attempt — grade discarded, activity still logged. Exam question messages must keep calling `logSessionQuestionMessage()` so session cleanup covers them.
